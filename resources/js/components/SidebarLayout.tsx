...
export const SidebarLayout: React.FC<LayoutProps> = ({ children }) => {
    ...
    return (
        <div className="bg-background text-on-background font-body-md min-h-screen">
            ...
            <aside className="fixed left-0 top-0 h-full hidden md:flex flex-col w-sidebar-width border-r border-outline-variant bg-surface z-50">
                <div className="p-6">
                ...
                <div className="p-4 border-t border-outline-variant space-y-2">
                    <div className="flex items-center gap-3 px-4 py-3 text-on-surface-variant">
                        ...
                        <span className="font-semibold truncate">{user?.name || 'Administrator'}</span>
                    </div>
                    <button
                        onClick={handleLogout}
                        className="w-full flex items-center gap-3 px-4 py-3 text-error hover:bg-error-container/20 rounded-lg transition-colors duration-200 ease-in-out"
                    >
                ...
                {navigationItems.slice(0, 5).map((item) => (
                    <Link
                        ...
                        className={`flex flex-col items-center justify-center transition-transform active:scale-90 ${
                            isActive(item.path)
                                ? 'bg-primary-container text-on-primary-container rounded-xl px-3 py-1'
            ...
            {/* Main Content Area */}
            <main className="md:ml-[260px] min-h-screen pt-16 pb-20 md:pb-8">
                {children}
            </main>
        </div>
    );
